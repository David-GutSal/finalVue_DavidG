...
function getPrecioMedio(idModelo) {
  ...
  if (!vehiculosDelModelo.length) {
    return 0
  }

  const suma = vehiculosDelModelo.reduce(
    (acc, v) => acc + v.precioDia,
    0
  )
...
async function guardarExtra(modelo) {
  ...
  const modeloActualizado = {
    ...modelo,
    extraPorModelo: Number(extra)
  }

  await axiosInstance.put(
    'http://localhost:3000/modelos/' + modelo.id,
    modeloActualizado
  )
  ...
  if (index !== -1) {
    modelos.value[index] = modeloActualizado
  }
  ...
  <div>
    ...
    <div v-if="idMarcaSeleccionada">
      <table class="table table-bordered table-hover">
        ...
        <tbody>
          <tr
            v-for="modelo in modelosDeMarca"
            :key="modelo.id"
          >
            <td>{{ modelo.modelo }}</td>

            <td>
              {{ getPrecioMedio(modelo.id).toFixed(2) }}€/día
            </td>
